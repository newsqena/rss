...
# إعداد جلسة اتصال دائمة لمحاكاة المتصفح
...
def extract_article(link):
    try:
        # استخدام الـ session بدلاً من طلب منفرد
        r = session.get(link, timeout=25)
        r.encoding = 'utf-8'
        soup = BeautifulSoup(r.text, "html.parser")
        
        # 1. سحب النص من كلاس entry
...
def upload_to_cloudinary_pro(image_url):
    ...
    try:
        # تحميل الصورة عبر الجلسة المؤمنة
...
def main():
    ...
    try:
        ...
        # جلب الـ RSS عبر الـ session لتجاوز الحماية
        rss_response = session.get(RSS_URL, timeout=25)
        feed = feedparser.parse(rss_response.content)
        
        published_links = load_history()
        print(f"✅ Feed loaded. Found {len(feed.entries)} items.")
        ...
        for entry in feed.entries:
            ...
            if not text or len(text) < 100: 
                print(f"⚠️ Content extraction failed for: {entry.title}")
                continue
